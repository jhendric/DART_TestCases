...
# GSR
# this is a variant of advance_model.csh that is used for downscaling. Only
# coding to add a single downscaled domain
# the wrfinput file is ready to go - don't need to do anything other than 
# prep LBC and run WRF
# resuing the num_states argument (second in list of arguments passed to this
# script) to now be the num_domains (but really only expecting this to be 2)
...
if ( $process == 0 ) then
   ...
   if ( ! -x ${CENTRALDIR}/WRF_RUN/da_wrfvar.exe ) then
      ...
      if ( ! -x update_wrf_bc ) then
         ...
         set SPEC_BC = `grep specified ${CENTRALDIR}/downscale.namelist.input | grep true | wc -l`
         if ( $SPEC_BC > 0 ) then
...
set output_file_line_a = 3
set output_file_line_b = 4

# MULTIPLE DOMAINS - need a way to tell this shell script if there are multiple wrf domains in the analysis

#while($state_copy <= $num_states)     # MULTIPLE DOMAINS - we don't expect advance model to run more than one member anymore. Reused num_states for # domains
   ...
   set output_file_a   = `head -$output_file_line_a   ${CENTRALDIR}/${control_file} | tail -1`
   set output_file_b   = `head -$output_file_line_b   ${CENTRALDIR}/${control_file} | tail -1`
...
#   ${REMOVE} dart_wrf_vector
  ...
  set stuff_vars =   ( U V PH T MU QVAPOR QCLOUD QRAIN QICE QSNOW QGRAUP QNICE QNRAIN H_DIABATIC \
...
#     ncks -A -v ${stuff_str} ../${input_file}${dchar} wrfinput_d${dchar}    #MUTLIPLE DOMAINS - input file is incomplete file name?
     @ dn++
     echo $dn $num_domains $dchar
  end  
...
#REALTIME  run unipost and extract ops in the bg
#   ${CENTRALDIR}/run_unipost_in ${ensemble_member} ${datea} >& post_out.txt &
#   cp ${CENTRALDIR}/extract_wrfin.ncl .
#   set cmd1 = "ncl 'mem_num=${ensemble_member}' 'date_in=${datea}' ${CENTRALDIR}/advance_temp${ensemble_member}/extract_wrfin.ncl"
#   cat >! nclrun.out << EOF
#   $cmd1 
#EOF
#   chmod +x nclrun.out
#   ./nclrun.out &
   ...
   echo "this is a run with ${MY_NUM_DOMAINS} domains"
...
## run the replace_wrf_fields utility to update the static fields
   if ( $USE_REPLACE ) then
# this should include other domains if needed
...
#         sed -f script.sed ${CENTRALDIR}/namelist.input.3dvar >! namelist.input  # we don't need this
...
EOF
      ...
      sed -f script.sed ${CENTRALDIR}/downscale.namelist.input >! namelist.input
...
#REALTIME
# run unipost on forecast 
#   cp ${CENTRALDIR}/extract_wrfout.ncl .
#   set cmd2 = "ncl 'mem_num=${ensemble_member}' 'date_in=${datea}' ${CENTRALDIR}/advance_temp${ensemble_member}/extract_wrfout.ncl"
#   ${REMOVE} nclrun.out
#   cat >! nclrun.out << EOF
#   $cmd2
#EOF
#   chmod +x nclrun.out
# assuming that the extract script will run faster than unipost
#   ./nclrun.out  &
# forecast date
#   ${CENTRALDIR}/run_unipost_out ${ensemble_member} ${datea} >& post_out2.txt 
...
# test for this, and wait if the zip operation is not yet finished
   ...
   ##############################################
#REALTIME
# move the diag, tar up the grib files, move to central_dir/POST
#   ${MOVE} ${CENTRALDIR}/advance_temp${ensemble_member}/mem*.nc ${CENTRALDIR}/POST/
#   ${MOVE} ${CENTRALDIR}/advance_temp${ensemble_member}/Omem*.nc ${CENTRALDIR}/OPOST/
#   tar -cvf post_${datea}_mem${ensemble_member}.tar  ./mem${ensemble_member}/WRF*
#   tar -cvf Opost_${datea}_mem${ensemble_member}.tar  ./Omem${ensemble_member}/WRF*
#   ${MOVE} post_${datea}_mem${ensemble_member}.tar ${CENTRALDIR}/POST/post_${datea}_mem${ensemble_member}.tar
#   ${MOVE} Opost_${datea}_mem${ensemble_member}.tar ${CENTRALDIR}/OPOST/Opost_${datea}_mem${ensemble_member}.tar

   cd ${CENTRALDIR}/advance_temp${ensemble_member}
   # withdraw LSM data to use in next cycle
...
set extract_vars_a = ( U V W PH T MU QVAPOR QCLOUD QRAIN QICE QSNOW QGRAUP QNICE QNRAIN H_DIABATIC \
                       U10 V10 T2 Q2 PSFC TH2 TSLB SMOIS TSK RAINC RAINNC GRAUPELNC HAILNC )
set extract_vars_b = ( U V W PH T MU QVAPOR QCLOUD QRAIN QICE QSNOW QGRAUP QNICE QNRAIN H_DIABATIC \
                       U10 V10 T2 Q2 PSFC TH2 TSLB SMOIS TSK RAINC RAINNC GRAUPELNC HAILNC REFL_10CM VT_DBZ_WT )
  set num_vars_a = $#extract_vars_a
  set num_vars_b = $#extract_vars_b
  set extract_str_a = ''
  set extract_str_b = ''
  set i = 1
  while ( $i < $num_vars_a )
  ...
  set extract_str_a = `echo ${extract_str_a}$extract_vars_a[$num_vars_a]`
  set i = 1
  while ( $i < $num_vars_b )
     set extract_str_b = `echo ${extract_str_b}$extract_vars_b[$i],`
     @ i ++
  end
  set extract_str_b = `echo ${extract_str_b}$extract_vars_b[$num_vars_b]`
...
#  while ( $dn <= $num_domains )
#     set dchar = `echo $dn + 100 | bc | cut -b2-3`
     ncks -O -v ${extract_str_a} wrfinput_d01 ../${output_file_a}   # MULTIPLE DOMAINS
     ncks -O -v ${extract_str_b} wrfinput_d02 ../${output_file_b}   # MULTIPLE DOMAINS includes state for radar assimilation
#     @ dn++
#  end
...
#      end
#   endif
# GSR moving this to here to make sure all the pieces are done
   ...
   # DMODS - don't expect to ever run multiple integrations from a single call to advance_model
#   @ state_copy ++
