...
# run unipost and extract ops in the bg
...
# The EOF on the line above MUST REMAIN in column 1.
         ...
         sed -f script.sed ${CENTRALDIR}/namelist.input.3dvar >! namelist.input
...
#   cd ${CENTRALDIR}/advance_temp${ensemble_member}/postprd
...
set extract_vars = ( U V W PH T MU QVAPOR QCLOUD QRAIN QICE QSNOW QGRAUP QNICE QNRAIN H_DIABATIC \
                     U10 V10 T2 Q2 PSFC TH2 TSLB SMOIS TSK RAINC RAINNC GRAUPELNC HAILNC )
  set num_vars = $#extract_vars
  set extract_str = ''
  set i = 1
  while ( $i < $num_vars )
     set extract_str = `echo ${extract_str}$extract_vars[$i],`
     @ i ++
  end
  set extract_str = `echo ${extract_str}$extract_vars[$num_vars]`
  echo ${extract_str}
...
# MULTIPLE DOMAINS - loop through wrf files that are present
  ...
  while ( $dn <= $domains )
     set dchar = `echo $dn + 100 | bc | cut -b2-3`
     ncks -O -v ${extract_str} wrfinput_d${dchar} ../${output_file}${dchar}   # MULTIPLE DOMAINS - output file is incomplete filename?
  end
...
# GSR moving this to here to make sure all the pieces are done
   ...
   # DMODS - don't expect this to ever be run anymore
   @ state_copy ++
