...
!
! $Id: sys_sim201.f90 11289 2017-03-10 21:56:06Z hendric@ucar.edu $

program sys_sim201

! MODIFIED Sep12 2003 from sys_sim5 to look at uncertainty in motion
! of mean in observation space with hierarchical ensembles.
!
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/assimilation_code/programs/system_simulation/sys_sim201.f90 $"
...
!!!read(*, *) y_p
! Make prior value always 0 to cut down a parameter
y_p = 0.0
write(*, *) 'Input variance of prior variance from groups'
read(*, *) var_of_var
write(*, *) 'Input variance of observing instrument'
read(*, *) sigma_y_o
!!!write(*, *) 'input value of observation '
!!!read(*, *) y_o
! This also doesn't matter; make it 1.0
y_o = 1.0
...
write(*, *) '-----------------'
write(*, *) 'normalized sample var - mean var ', &
   real((sample_var - sum_var / n_samples) / (y_o - y_p)**2)
write(*, *) '-------------------------------'
write(*, *) 'ratio of mean_var to prior var  ', real(mean_var /sigma_y_p) 
write(*, *) 'expected ratio of mean to prior ', real(inf_var / sigma_y_p) 
write(*, *) '-------------------------------'
