...
!
! $Id: sys_sim105.f90 11289 2017-03-10 21:56:06Z hendric@ucar.edu $
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/assimilation_code/programs/system_simulation/sys_sim105.f90 $"
...
do i = 1, 200
   sd_ratio = (i - 1.) / 100.0
   call sub_sim105(sd_ratio, n, n_samples, alpha)
   write(*, *) 'sd_ratio, alpha ', sd_ratio, alpha
!!!   if(alpha <= 0.0) stop
end do

end program sys_sim105



subroutine sub_sim105(sd_ratio, n, n_samples, alpha)

! sys_sim105 adds in an ad hoc measure of additional uncertainty that comes
...
! CAN I STOP SOME OF THE SAMPLING PROBLEMS LIKE THIS?

!write(*, *) i, 'ratio ', ratio

! Now, draw a single value from this distribution
   sample_mean(i) = random_gaussian(r, base_mean, ratio)

! MAYBE I CAN STOP SAMPLNG PROBLEMS HERE???
   if(sample_mean(i) > 10000.0) sample_mean(i) = 10000.0
   if(sample_mean(i) < -10000.0) sample_mean(i) = -10000.0
...
! Compute the sum of the absolute errors for this value of alpha
   sum_abs_error = 0.0
   do i = 1, n_samples
      sum_abs_error = sum_abs_error + abs(alpha * sample_mean(i) - 1.0) 
   end do
