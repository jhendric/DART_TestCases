...
!
! $Id: sys_sim402.f90 11289 2017-03-10 21:56:06Z hendric@ucar.edu $
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/assimilation_code/programs/system_simulation/sys_sim402.f90 $"
...
end program sys_sim402



subroutine sub_sim402(sd_ratio, n, n_samples, alpha)

! sys_sim402 adds in an ad hoc measure of additional uncertainty that comes
...
double precision :: mean_abs_error(num_tries), base_mean
...
double precision, allocatable :: ratio(:), x_var(:)
...
double precision :: mean_ratio, sum_ratio, median_ratio
double precision :: garb, table(600)
...
allocate(ratio(n_samples), rnum(n))
...
! Compute the sample mean and variance
   ...
   ratio(i) =  sqrt(sample_sigma_y_p) / abs(base_mean)
   if(ratio(i) > 10000.0) ratio(i) = 10000.0
   sum_ratio = sum_ratio + ratio(i)

end do

! Now, lookup the value of the mean ratio in the full table
mean_ratio = sum_ratio / n_samples
!write(*, *) 'ratios ', sd_ratio, mean_ratio
!ratio = sort(ratio)
!median_ratio = ratio(n_samples / 2)

! What about the median ratio


! Look up the appropriate value in the full damped table
open(unit = 47, file = 'smoothed_damped_base')
do j = 1, 600
   read(47, *) garb, table(j)
end do

index = int(100 * mean_ratio) + 2
if(index > 600) then
   alpha = 0.0
else
   alpha = table(index)
