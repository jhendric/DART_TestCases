...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/assimilation_code/programs/obs_diag/threed_sphere/obs_diag.f90 $"
...
if (is_vertical(obslocation, "SCALE_HEIGHT")               .and. &
   ...
   (ob_defining_vert(flavor)  >     0       ) ) then
        write(string1,'(''obs '', i8, '' type '', i3, &
        & '' changing from '', i2, '' to scale_height - def by obs '',i8)') &
        keys(obsindex), flavor, which_vert(flavor), ob_defining_vert(flavor)
   call error_handler(E_WARN,'CheckVertical',string1,source,revision,revdate)
endif
...
do i = 2,SIZE(mydata)
   ...
   elseif (any(mydata(i) == temparray(1:n))) then
      ...
      call error_handler(E_MSG,'IRemoveDuplicates', &
