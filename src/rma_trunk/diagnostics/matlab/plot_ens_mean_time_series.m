function plot_ens_mean_time_series(input_file)
...
% plot_ens_mean_time_series('output.nc')

%% DART software - Copyright UCAR. This open source software is provided
% by UCAR, "as is", without charge, subject to all terms of use at
% http://www.image.ucar.edu/DAReS/DART/DART_download
%
% DART $Id: plot_ens_mean_time_series.m 11871 2017-08-03 18:56:00Z jla@ucar.edu $
...
switch lower(pinfo.model)
   ...
   case {'fms_bgrid'}
      ...
      pinfo = GetBgridInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'cam'}

      pinfo = GetCamInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'wrf'}

      pinfo = GetWRFInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'pe2lyr'}

      pinfo = GetPe2lyrInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'mitgcm_ocean'}

      pinfo = GetMITgcm_oceanInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'mpas_atm'}

      pinfo = GetMPAS_ATMInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'sqg'}

      pinfo = GetSqgInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');

   case {'pop'}

      pinfo = GetPOPInfo(pinfo, input_file, 'PlotEnsMeanTimeSeries');
...
% $URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/diagnostics/matlab/plot_ens_mean_time_series.m $
% $Revision: 11871 $
% $Date: 2017-08-03 12:56:00 -0600 (Thu, 03 Aug 2017) $
