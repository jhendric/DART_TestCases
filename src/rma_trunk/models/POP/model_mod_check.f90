...
!
! $Id: model_mod_check.f90 11243 2017-03-08 21:35:09Z nancy@ucar.edu $
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/POP/model_mod_check.f90 $"
...
if (test1thru > 2) then
   ...
   call get_pop_restart_filename(pop_input_file)

   write(*,*)'Reading file from  '//trim(pop_input_file)
   write(*,*)'Writing data into '//trim(output_file)

   call restart_file_to_sv(pop_input_file, statevector, model_time)
...
if (test1thru > 3) then
   ...
   if ( .not. file_exist(dart_input_file) ) then
      ...
      write(string2,*) "Run 'pop_to_dart' to generate ",trim(dart_input_file)
...
integer :: ilon, jlat, kvert, nfailed, nland
...
if (nlon*nlat*nvert - nland == 0) then
   ...
   write(*,*) 'total interpolations  : ', nlon*nlat*nvert - nland
