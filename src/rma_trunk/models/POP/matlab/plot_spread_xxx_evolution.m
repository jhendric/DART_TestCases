...
%
% USAGE: plotdat = plot_spread_xxx_evolution(fname, copystring [,varargin]);
%
...
%
%
% EXAMPLE 1 - plot the spread and totalspread on the same axis.
%
...
%
%
% EXAMPLE 2 - plot the spread and bias on the same axis - for just one observation type.
%
% fname      = 'obs_diag_output.nc';
% copystring = 'bias';
% obstype    = 'MOORING_TEMPERATURE';
% plotdat    = plot_spread_xxx_evolution(fname, copystring, obstype);

%% DART software - Copyright UCAR. This open source software is provided
% by UCAR, "as is", without charge, subject to all terms of use at
% http://www.image.ucar.edu/DAReS/DART/DART_download
%
% DART $Id: plot_spread_xxx_evolution.m 10990 2017-02-02 23:39:18Z thoar@ucar.edu $
...
for ivar = 1:plotdat.nvars
   ...
   lgfname = sprintf('%s_spread_%s_obscount.txt',plotdat.varnames{ivar},plotdat.copystring);
   fprintf('Removing %s from the current directory.\n',lgfname)
   ...
   for ilevel = 1:plotdat.nlevels
      ...
      plotdat.ges_sprd  = guess(:,plotdat.sprdindex,  ilevel,:);
      plotdat.anl_sprd  = analy(:,plotdat.sprdindex,  ilevel,:);
...
mg = plotdat.ges_sprd(:,:,:,plotdat.region);
ma = plotdat.anl_sprd(:,:,:,plotdat.region);
sprd = reshape([mg ma]',2*plotdat.Nbins,1);
...
nobs = sum(nobs_used);
if ( nobs > 1 )
   mean_pr_sprd  = mean(mg(isfinite(mg)));
   mean_po_sprd  = mean(ma(isfinite(ma)));
...
string_sprd   = sprintf('%s pr=%.5g, po=%.5g','spread', mean_pr_sprd, mean_po_sprd);
string_other  = sprintf('%s pr=%.5g, po=%.5g', plotdat.copystring, ...
                       mean_pr_other, mean_po_other);
plotdat.subtitle = sprintf('%s     %s     %s',plotdat.myregion,string_sprd, string_other);
...
h1 = plot(t,sprd,'k+-',t,other,'ro-','LineWidth',plotdat.linewidth);
h = legend(h1,'spread', plotdat.copystring);
...
switch lower(plotdat.copystring)
   case 'bias'
      ...
      plotdat.ylabel{2} = sprintf('spread and %s (%s)',plotdat.copystring,plotdat.biasconv);
   otherwise
      plotdat.ylabel{2} = sprintf('spread and %s',plotdat.copystring);
end
...
bob  = [y.ges_copy(:) ; y.ges_sprd(:) ; ...
        y.anl_copy(:) ; y.anl_sprd(:) ];
...
% $URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/POP/matlab/plot_spread_xxx_evolution.m $
% $Revision: 10990 $
