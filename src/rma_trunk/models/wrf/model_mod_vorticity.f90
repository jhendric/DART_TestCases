...
!
! $Id: model_mod_vorticity.f90 11686 2017-05-31 20:42:33Z nancy@ucar.edu $
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/wrf/model_mod_vorticity.f90 $"
character(len=32 ), parameter :: revision = "$Revision: 11686 $"
character(len=128), parameter :: revdate  = "$Date: 2017-05-31 14:42:33 -0600 (Wed, 31 May 2017) $"
...
!--------------------------------------------------------------------------
!> Vorcity forward operator. 
subroutine vorticity_forward_operator(use_old_vortex, fld, wrf, id, i, j, k, obs_kind, dxm, dx, dy, dym, uniquek, ens_size, state_ens_handle)

logical,             intent(in) :: use_old_vortex
integer,             intent(in) :: ens_size
type(ensemble_type), intent(in) :: state_ens_handle
...
real(r8) :: ugrid, vgrid ! ens_size?

logical :: debug = .false.

logical  :: is_lev0(ens_size)
real(r8) :: zloc(ens_size)
integer  :: uk !> loop variable
...
UNIQUEK_LOOP: do uk = 1, size(uniquek)
   ...
   ! Check to make sure retrieved integer gridpoints are in valid range
   if (.not. ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%type_t ) .and. &
               boundsCheck( j, wrf%dom(id)%polar,      id, dim=2, type=wrf%dom(id)%type_t ) .and. &
...
end subroutine vorticity_forward_operator
...
do i1 = circ_xmin-1, circ_xmax+1
   ...
   do i2 = circ_ymin-1, circ_ymax+1
      ...
      do k2 = 1, wrf%dom(id)%var_size(3,wrf%dom(id)%type_t)
         z1d(k2, :) = model_pressure_t(i1,i2,k2,id, state_ens_handle, ens_size)
      enddo
      z1d(0) = z1d(1)
...
end subroutine vorticity_one

!--------------------------------------------------------------------------
!> vorticity_two
subroutine vorticity_two()
...
end subroutine vorticity_two

!--------------------------------------------------------------------------
!> vorticity_three
subroutine vorticity_three()
...
end subroutine vorticity_three
...
! $Id: model_mod_vorticity.f90 11686 2017-05-31 20:42:33Z nancy@ucar.edu $
! $Revision: 11686 $
! $Date: 2017-05-31 14:42:33 -0600 (Wed, 31 May 2017) $
