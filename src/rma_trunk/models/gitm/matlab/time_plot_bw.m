...
% DART $Id: time_plot_bw.m 10990 2017-02-02 23:39:18Z thoar@ucar.edu $
...
%distribution
if ~isempty(distr)
    hdi = patch( di/max(di)*dsh(1)+dsh(2), s*ddom, .55*[1,1,1],'edgecolor',.45*[1,1,1],'FaceAlpha',a,'linestyle','--'); %,'FaceAlpha',0.5
...
if ~isempty(tmc)
    ...
    meaccol=sum(meaccol,2);
    meaccol=meaccol-min(meaccol);
    meaccol=meaccol/max(meaccol);
    meaccol=repmat(meaccol,1,3);
...
if ~isempty(teno)
    heno=plot(teno,eno,'+', 'color',.4*[1,1,1]);
    h=[h heno];
    la{1,end+1}=laeno;
end
...
%measurement spread
if ~isempty(tmd)
    hmd=patch([tmd(:)' fliplr(tmd(:)')],s*[meas(:)'+mead(:)' fliplr(meas(:)'-mead(:)')],.2*[1,1,1],'edgecolor',.3*[1,1,1],'FaceAlpha',a,'linestyle','--');%,'FaceAlpha',0.5
    h=[h hmd];
    la{1,end+1}=lamd;
end
...
%ensemble spread
if ~isempty(tes)
    hspr=patch([tenRO(:)' fliplr(tenRO(:)')],s*[meanRO(:)'+sprRO(:)' fliplr(meanRO(:)'-sprRO(:)')],.5*[1,1,1],'edgecolor',.4*[1,1,1],'FaceAlpha',a);%,'FaceAlpha',0.5
    h=[h hspr];
    la{1,end+1}=laspr;
end
...
%ensemble mean
if ~isempty(ten)
    hmean=plot(tenRO,s*meanRO,'color',.7*[1,1,1],'linewidth',2);
    h=[h hmean];
    la{1,end+1}=lamean;
end
...
%measurement
if ~isempty(tms)
    hms=plot(tms,s*meas,'--','color',.2*[1,1,1],'linewidth',2);
    h(1,end+1)=hms;
    la{1,end+1}=lams;
end
...
% $URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/gitm/matlab/time_plot_bw.m $
% $Revision: 10990 $
