...
% DART $Id: time_plot_old.m 10990 2017-02-02 23:39:18Z thoar@ucar.edu $
...
if ~isempty(distr)
    hdi = patch( di/max(di)*dsh(1)+dsh(2), ddom, 'g','edgecolor',[0 .5 0]); % di/max(di)*dsh(1)+dsh(2) makes it not to scale, but helps to make it more visible
    h=[h hdi];
    la{1,end+1}=lad;
end
...
if ~isempty(tes)
    hspr=patch([tenRO fliplr(tenRO)],[meanRO+sprRO fliplr(meanRO-sprRO)],'b','facecolor',[135 206 250]/255,'edgecolor','b');
    h=[h hspr];
    la{1,end+1}=laspr;
end

if ~isempty(ten)
    hmean=plot(tenRO,meanRO,'b','linewidth',2);
    h=[h hmean];
    la{1,end+1}=lamean;
end
...
if ~isempty(tms)
    hms=stairs(tms,meas,'r','linewidth',2);
    h(1,end+1)=hms;
    la{1,end+1}=lams;
end
...
% $URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/gitm/matlab/time_plot_old.m $
% $Revision: 10990 $
