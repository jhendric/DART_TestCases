#3dplot
al = 0.3 #transparency
vi,va = 0,90 #specify zlim and colorlimits
...
#GPS data
    ...
    y = LatD[timeD1==ti]
    z = VtecD[timeD1==ti]
    scatD = ax.scatter(x, y, z, c=z, cmap=plt.cm.jet, s=20, vmin=vi, vmax=va, alpha=al) #cmap=plt.cm.hot
    plt.colorbar(scatD);
    
#legend
    p1 = plt.Rectangle((0, 0), 1, 1, fc='b') #normal legend doesn't work for mplot3d
    p2 = plt.Rectangle((0, 0), 1, 1, fc='r')
    p3 = plt.Rectangle((0, 0), 1 ,1, fc='orange')
...
#labels, etc
    ...
    #    fig.canvas.draw()    
    plt.savefig('dots' + str(tii).zfill(2) + '.png', bbox_inches=0)
    if tii==0:   plt.savefig('dots00.eps', bbox_inches=0)
    tii += 1
    #plt.ion()

print subprocess.check_output('convert dots*.png dots00.gif', shell=True) #convert to animated gif 

# DART $Id: plot_gps_3d.py 11001 2017-02-03 23:08:55Z thoar@ucar.edu $
# from Alexey Morozov

# <next few lines under version control, do not edit>
# $URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/gitm/python/plot_gps_3d.py $
# $Revision: 11001 $
