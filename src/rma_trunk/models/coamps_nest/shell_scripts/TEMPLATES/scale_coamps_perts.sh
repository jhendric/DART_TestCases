...
#
# DART $Id: scale_coamps_perts.sh 6256 2013-06-12 16:19:10Z thoar $
#
###############################################################################
#
# AUTHOR:   P. A. Reinecke
...
LOG_DIR=${scrDir}/log
scale_perts=${scriptDir}/scale_coamps_perts.${dtg}.sh
...
ensdir=\`printf \${COAMPS_DATA}/analyses 1\`
cd \${ensdir}
flist=( \`ls *\${dtg}*analfld *\${dtg}*fcstfld\` )
nfiles=\${#flist[@]}

cd \${WORKDIR}
var_file='./scale_coamps_perts.vars'
...
m
EOF_NL
\${NL_STRIP} ${scrDir}/namelist \${var_file} scale_coamps_perts
rm -f \${var_file}

cat >> scale_coamps_perts.nml << EOF_NL
&field_proc
...
\${MPI_CMD} \${DART_BIN}/scale_coamps_perts 
rm -f scale_coamps_perts.nml
...
# $URL: https://svn-dares-dart.cgd.ucar.edu/DART/branches/rma_trunk/models/coamps_nest/shell_scripts/TEMPLATES/scale_coamps_perts.sh $
# $Revision: 6256 $
